class ApiResponse {
    ...
    // Static factory method for error. `errors` carries field-level validation
    // messages (e.g. { title: "Title is required." }) surfaced under `data.errors`.
    static error(message, errors = null) {
        return new ApiResponse(false, message, errors ? { errors } : null, new Date());
    }
